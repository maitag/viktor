@:access(ViktorT)
class TestViktorT extends haxe.unit.TestCase
{
	public function testViktorT() {
		...
		#if viktor_safe
		// error: null
		assertTrue( try { v.add(null); false; } catch (e:Dynamic) true );
		// error: key not exist
		...
		assertEquals( v.get(2), null );
		assertEquals( v.get(3), "d" );
		assertEquals( v.get(4), null );

		#if viktor_safe
		// error: null
		assertTrue( try { v.key(null); false; } catch (e:Dynamic) true );
		#end
		...
		#if viktor_safe
		// error: null
		assertTrue( try { v.set(2, null); false; } catch (e:Dynamic) true );
		...
		assertEquals( v.add("a"), 2 );
		assertEquals( v.add("b"), 1 );
		assertEquals( v.add("c"), 0 );
		assertEquals( v.length, 3 );
		assertEquals( v.pos, 3 );
		assertEquals( v.posFree, -1 );
		assertEquals( v.toString(), "[0=>c,1=>b,2=>a]" );
