@:access(ViktorInt)
class TestViktorInt extends haxe.unit.TestCase
{
	public function testViktorInt() {

		var v = new ViktorInt(5);
		...
		#if viktor_safe
		// error: -1
		assertTrue( try { v.add(-1); false; } catch (e:Dynamic) true );
		// error: key not exist
		...
		assertEquals( v.get(4), -1 );

		#if viktor_safe
		// error: -1
		assertTrue( try { v.key(-1); false; } catch (e:Dynamic) true );
		#end
		...
		#if viktor_safe
		// error: -1
		assertTrue( try { v.set(2, -1); false; } catch (e:Dynamic) true );
		...
		assertEquals( v.add(111), 2 );
		assertEquals( v.add(222), 1 );
		assertEquals( v.add(333), 0 );
		assertEquals( v.length, 3 );
		assertEquals( v.pos, 3 );
		assertEquals( v.posFree, v.size -1 );
		assertEquals( v.toString(), "[0=>333,1=>222,2=>111]" );
